...
double VoxelKey::Span(const las::LasHeader &header) const
{
    return header.Span() * std::pow(2, -d);
}

double VoxelKey::GetSpanAtDepth(int32_t d, const las::LasHeader &header)
{
    return VoxelKey(d, 0, 0, 0).Span(header);
}
